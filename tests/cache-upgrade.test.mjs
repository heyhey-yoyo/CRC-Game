...
test('运行时JSON内容地址与SW预缓存版本一致', async () => {
  const calls = [], window = {};
  const fetch = async (url, options) => {
    calls.push({ url, options });
    const file = new URL(url, 'https://test.invalid/').pathname.slice(1);
    return { ok: true, json: async () => JSON.parse(read(file)) };
  };
  vm.runInNewContext(read('js/content-loader.js'), { window, fetch });
  await window.CRC_CONTENT_LOADER.load();
  assert.equal(calls.length, 4);
  for (const call of calls) {
    assert.equal(new URL(call.url, 'https://test.invalid/').searchParams.get('v'), revision);
    assert.equal(call.options.cache, 'no-cache');
    assert.ok(read(swPath).includes(call.url));
  }
});
